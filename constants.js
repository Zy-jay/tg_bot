...
const QUERIES = {
  ...
  checkIfCalledFromChannelByTokenAndPrelaunch:
...
const social_network = async (tokenAddress, chainId) => {
  const data = await fetch(
    `https://api.pinksale.finance/api/v1/pool/detail?chainId=${chainId}&tokenAddress=${tokenAddress}`
  ).then((res) => res.json());
  if (data.error || !data.poolDetails) {
    ...
    if (pairs.errorCode) {
      const data = await fetch(
        `https://dex-api-production.up.railway.app/v1/dex/pair/poolAddress/${tokenAddress}?chainId=${chainId}`
      ).then((res) => res.json());
    ...
    const data = await fetch(
      `https://dex-api-production.up.railway.app/v1/dex/pair/poolAddress/${pairs?.data?.pairs[0]?.address}?chainId=${chainId}`
    ).then((res) => res.json());
    if (data?.error || !data?.pairs?.data?.baseTokenInfo) return null;
    const res = data?.pairs?.data?.baseTokenInfo;
    const resObj = {
