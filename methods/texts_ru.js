...
${formated.prelaunchCalls
  ...
  const launched = await Promise.all(
    formated.result.map(async (item, i) => {
      ...
      for (let index = 0; index < item.length; index++) {
        ...
        const res = `${
          elementNumber + 1
        }. <a href="https://t.me/${escapeHtmlEntities(
          e.channelInnerLink
        )}/${escapeHtmlEntities(e.message_id)}">${escapeHtmlEntities(
          e.channelTitle
        )}</a>: ${
          ...
          currentROI == 0 || isNaN(currentROI)
            ? "🍀"
            : parseFloat(currentROI) >= 1
            ? currentROI + "x"
            : "-" + (1 - parseFloat(currentROI) * 100) + "%"
        }🔹\n`;
        if (res[0] == ",") {
          res = res.slice(1);
        }
  ...
  const networks = await social_network(
    tokenInfo.address,
    tokenInfo.chain == "ether" ? 1 : 56
  );
  let website = "";
  ...
  if (networks) {
    ...
    if (tgUrl) {
      ...
      tg = networks?.telegram
        ? ` | <a href="${networks?.telegram}">Telegram</a>`
        : "";
    }
    website = networks?.website
      ? `<a href="${networks?.website}">Сайт</a>`
      : "";
    git = networks?.github ? ` | <a href="${networks?.github}">Github</a>` : "";
    schat = networks?.sourceChat
      ? ` | <a href="${networks?.sourceChat}">SourceChat</a>`
      : "";
    youtube = networks?.youtube
      ? ` | <a href="${networks?.youtube}">Youtube</a>`
      : "";
  } else {
    if (twitterUrl) {
      twitter = ` | <a href="${twitterUrl}">Twitter</a>`;
  ...
  const db_networks = await pool.query(
    `SELECT * FROM networks WHERE token_address = $1`,
    [tokenInfo.address]
  )?.rows;
  if (!db_networks?.token_address) {
    try {
      await pool.query(
        `INSERT INTO networks (token_address, twitter, telegram) VALUES ($1, $2, $3)`,
        [tokenInfo.address, twitterUrl, tgUrl]
      );
    } catch (e) {
      if (db_networks?.twitter) {
        ...
        if (twitterUrl) {
          await pool.query(
            `UPDATE networks SET twitter = $2 WHERE token_address = $1`,
            [tokenInfo.address, twitterUrl]
          );
        }
      }
      if (db_networks?.telegram) {
        ...
        if (tgUrl) {
          await pool.query(
            `UPDATE networks SET telegram = $2 WHERE token_address = $1`,
            [tokenInfo.address, tgUrl]
          );
        }
      }
    }
  ...
  return `<b>🟩TOTAL CALLS </b> ${escapeHtmlEntities(tokenInfo.key_name)} - ${
...
<b>Держатели:</b> ${
  ...
  } | <b>Отказ от контракта:</b> ${
    tokenDetailsForMessage.renounced ? "да" : "нет"
  }
...
<b>Подпишись на ${
    TELEGRAM.CHANNEL
  } чтобы первым найти перспективные токены</b>`;
}

async function getPreCallText(
  tokenInfo,
  channelInnerLink,
  channelTitle,
  message
...
async function getUpdateText(
  ...
  return `<b>🟩NEW CALL -</b> <a href="https://t.me/${escapeHtmlEntities(
    channelInnerLink
  )}/${escapeHtmlEntities(message.id)}">${escapeHtmlEntities(
    channelTitle
  )}</a> called ${escapeHtmlEntities(tokenInfo.key_name)}
