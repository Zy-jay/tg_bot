...
async function eventPrint(event, bot) {
  try {
    ...
    const channelsInBase = (
      await pool.query(QUERIES.getChannelsInBaseByBotNumber, [
        TELEGRAM.BOT_NUMBER,
      ])
    ).rows.map((row) => parseInt(row.channel_id, 10));
    const channelID = parseInt(
      message?.peer_id?.channel_id || message?.peer_id?.chat_id,
      10
    );
    ...
    const entitiesURLs = (message?.entities?.map((e) => e?.url) || []).filter(
      (e) => e
    );
    ...
    const matches = [...new Set(mixedText.match(regex) || [])];

    console.log("matches:", matches);

    for (const iterator of matches) {
      if (
        EXCEPTION_TOKENS.find(
      ...
      const tokenData = await (async () => {
        ...
        const pairData = await fetch(
          `https://api.dextools.io/v1/pair?chain=ether&address=${iterator}`,
          {
            headers: {
        ...
        const ethAddress =
          pairData.statusCode === 200
            ? pairData?.data?.token?.address
            : iterator;
        ...
        if (tokenData.statusCode === 200)
          return { tokenData: tokenData, pairData: pairData };

        // try to get BSC pair
        const bscPairData = await fetch(
          `https://api.dextools.io/v1/pair?chain=bsc&address=${iterator}`,
          {
            headers: {
              "X-API-Key": TOOLS.DEXTOOLS_API_KEY,
            },
        ...
        const bscAddress =
          bscPairData.statusCode === 200
            ? bscPairData?.data?.token?.address
            : iterator;
        const bscTokenData = await fetch(
          `https://api.dextools.io/v1/token?chain=bsc&address=${bscAddress}`,
          {
            headers: {
              "X-API-Key": TOOLS.DEXTOOLS_API_KEY,
            },
        ...
        if (bscTokenData.statusCode === 200)
      ...
      const tokenDataDexView = tokenData?.tokenData?.data?.address
        ? {}
        : await getTokenData(iterator);
      ...
      const pairAddress =
        tokenData?.bscPairData != undefined
      ...
      const tokenInfo = tokenData?.tokenData?.data?.address
        ...
        : {
            ...
            key_name:
              "$" +
              (tokenDataDexView?.token0Symbol ||
                tokenDataDexView?.baseTokenSymbol ||
                ""),
            address: (
              tokenDataDexView.token0 ||
              tokenDataDexView.baseToken ||
              ""
            ).toLowerCase(),
      ...
      const pairInfo = await fetch(
        `https://api.dextools.io/v1/pair?chain=${tokenInfo.chain}&address=${tokenInfo?.pairs?.[0]?.address}`,
        {
          headers: {
            "X-API-Key": TOOLS.DEXTOOLS_API_KEY,
          },
      ...
      const hasReleasedCall =
        (
          await pool.query(QUERIES.hasReleasedCallByTokenAndPrelaunch, [
            tokenInfo.address,
            false,
          ])
        ).rows[0] || false;

      const isPrelaunchInDextools =
        (pairInfo?.data?.metrics?.liquidity || 0) <= 0 ||
        !tokenData?.tokenData?.data?.pairs?.[0];
      const isPrelaunchInDexview = (tokenDataDexView.liquidity || 0) <= 0;

      const isPrelaunch =
        isPrelaunchInDextools && isPrelaunchInDexview && !hasReleasedCall;
      ...
      const tokenDetailsForMessage = {
        ...
        renounced:
          tokenData?.tokenData?.data?.audit?.is_contract_renounced !== undefined
            ? tokenData?.tokenData?.data?.audit?.is_contract_renounced
            : "no data",
        liquidity:
          pairInfo?.data?.metrics?.liquidity ||
          tokenDataDexView?.liquidity ||
          0,
        volume24:
          pairInfo?.data?.price24h?.volume ||
          tokenDataDexView?.newInformation?.volume24h ||
          0,
      };
      ...
      console.log(
        (await pool.query(QUERIES.getTokenIdByAddress, [tokenInfo.address]))
          .rows
      );

      const alreadyCalledFromThisChannel = (
        await pool.query(QUERIES.checkIfCalledFromChannelByTokenAndPrelaunch, [
          tokenInfo.address,
          channelID.toString(),
          isPrelaunch,
        ])
      ...
      console.log(
        "already called from this channel:",
        alreadyCalledFromThisChannel || false
      );
      if (alreadyCalledFromThisChannel) continue;

      const { channelInnerLink, channelTitle } = await (async () => {
        const savedData = (
          await pool.query(QUERIES.getSavedDataByChannelId, [
            channelID.toString(),
          ])
        ).rows[0];
      ...
      async function getTotal() {
        const allCalls = (
          await pool.query(QUERIES.getTotalCallsByTokenAddress, [
            tokenInfo.address,
          ])
        ).rows.sort(
        ...
        for (const call of allCalls) {
          const dbData = (
            await pool.query(QUERIES.getSavedDataByChannelId, [call.channel_id])
          ).rows[0];
        ...
        for (let index = 0; index < allCalls.length; index++) {
          ...
          call.ROI = getROI(
            tokens.filter((token) => token.id == call.token_id)[0].address,
            tokens.filter((token) => token.id == call.token_id)[0].chain ==
              "bsc"
              ? 56
              : 1,
            call.timestamp
        ...
        return channelsDetails.sort(
          (a, b) => parseInt(a.timestamp, 10) - parseInt(b.timestamp, 10)
        );
      }

      const tokenInBase = (
        await pool.query(QUERIES.getTokenDataByAddress, [tokenInfo?.address])
      ).rows[0];
      if (!tokenInBase) {
        await pool.query(QUERIES.insertToken, [
          tokenInfo?.name,
          tokenInfo?.key_name,
          tokenInfo?.address,
          tokenInfo?.chain,
        ...
        await pool.query(QUERIES.insertCall, [
          tokenInfo?.address,
          channelID.toString(),
          +new Date(),
        ...
        const totalMessage = await bot.telegram
          .sendMessage(
            TELEGRAM.CHANNEL,
        ...
        await pool.query(QUERIES.updateTokenTotalMessageId, [
          totalMessage.message_id,
          tokenInfo.address,
        ]);
      } else {
        await pool.query(QUERIES.insertCall, [
      ...
      const currentMaxCap = (
        await pool.query(QUERIES.getCurrentMaxMarketCapByTokenAddress, [
          tokenInfo.address,
        ])
      ).rows[0]?.max_market_cap;

      if (
        currentMaxCap < parseInt(tokenInfo.market_cap, 10) ||
        !currentMaxCap
      ) {
        await pool.query(QUERIES.updateTokenMaxMarketCap, [
          parseInt(tokenInfo.market_cap, 10) || null,
          tokenInfo.address,
        ]);
